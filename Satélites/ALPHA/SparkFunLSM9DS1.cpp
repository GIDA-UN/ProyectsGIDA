...
Attitude LSM9DS1::getAttitude() {
	if (gyroAvailable()) readGyro();
	if (accelAvailable()) readAccel();
	if (magAvailable())  readMag();

	float accel3D[3] = { calcAccel(ax), calcAccel(ay), calcAccel(az) };
	float gyro3D[3] = { calcAccel(gx), calcAccel(gy), calcAccel(gz) };
	float mag3D[3] = { calcAccel(mx),calcAccel(my), calcAccel(mz) };

	float roll = atan2(accel3D[1], accel3D[2]);
	float pitch = atan2(-accel3D[0], sqrt(accel3D[1] * accel3D[1] + accel3D[2] * accel3D[2]));
	float xAux = mag3D[0] * cos(pitch) + mag3D[1] * sin(pitch)*sin(roll) + mag3D[2] * sin(pitch)*cos(roll);
	float yAux = -mag3D[1] * cos(roll) + mag3D[2] * sin(roll);
	float yaw = atan2(yAux,xAux);

	Attitude result;
	result.roll = roll*180/PI;
	result.pitch = pitch * 180 / PI;
	result.yaw = yaw * 180 / PI;
	return result;
}
Attitude LSM9DS1::getAttitude(float dt) {
	if (gyroAvailable()) readGyro();
	if (accelAvailable()) readAccel();
	if (magAvailable())  readMag();
